...
commander
    ...
    .description('Recursively searches the <path> attempting to match the <pattern> e.g. C:/users/xyz/documents "some text.*and some other text"')
    .option("-a, --after <after>", "Match files created exclusively after this date/datetime." +
    os.EOL +
    `\tAcceptable formats (in order of precedence): ${os.EOL}${dateFmts
        .map(f => `\t\t[${f}]`)
        .join(os.EOL)}`, s => moment(s, dateFmts).toDate())
    .option("-b, --before <before>", "Match files created exclusively before this date/datetime." +
    os.EOL +
    `\tAcceptable formats (in order of precedence): ${os.EOL}${dateFmts
        .map(f => `\t\t[${f}]`)
        .join(os.EOL)}`, s => moment(s, dateFmts).toDate())
    ...
    let p = pathstr || "";
    if (typeof p !== "string") {
        console.log(chalk_1.default.red("Usage: pdfsearch <path> <pattern> [options]"));
    ...
    try {
        if (!pattern || typeof pattern !== "string") {
            console.log(chalk_1.default.bold.redBright("No pattern given! Usage: pdfsearch <path> <pattern> [options]"));
            process.exit(1);
        }
        if (pattern.includes(" ")) {
    ...
    const SearchFile = async (name, path, stats, data) => {
        if (opts.after &&
            !moment(opts.updated ? stats.ctime : stats.birthtime).isAfter(opts.after)) {
            return;
        }
        if (opts.before &&
            !moment(opts.updated ? stats.ctime : stats.birthtime).isBefore(opts.before)) {
            return;
        }
    ...
    return WalkDir(opts, opts.path, SearchFile)
        .catch(e => {
        console.log("Unhandled error! Please report at github.com/sammons/pdf-search", e);
        process.exit(1);
    })
        .then(() => {
